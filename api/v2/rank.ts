...
import { formatOrdinals } from "../../src/utils/format";
...
export default async function handler(request, response) {
  ...
  const contributorStats = (data as CommitsTopResponse).users.find(
  ...
  if (!contributorStats) {
    return response
      .status(404)
      .json({ status: "error", result: `User not found within ${location}` });
  }
  ...
  return response
    ...
    .json({
      ...
      result: {
        ...contributorStats,
        ordinalRank: formatOrdinals(contributorStats.rank),
      },
    });
}
